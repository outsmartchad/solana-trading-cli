...
exports.TickUtils = exports.TICK_ARRAY_BITMAP_SIZE = exports.TICK_ARRAY_SIZE = void 0;
const bn_js_1 = __importDefault(require("bn.js"));
const constants_1 = require("./constants");
const pda_1 = require("./pda");
const tickQuery_1 = require("./tickQuery");
exports.TICK_ARRAY_SIZE = 60;
exports.TICK_ARRAY_BITMAP_SIZE = 512;
class TickUtils {
    ...
    static getTickOffsetInArray(tickIndex, tickSpacing) {
        ...
        if (offsetInArray < 0 || offsetInArray >= exports.TICK_ARRAY_SIZE) {
            throw new Error('tick offset in array overflow');
        }
        return offsetInArray;
    }
    static getTickArrayBitIndex(tickIndex, tickSpacing) {
        const ticksInArray = tickQuery_1.TickQuery.tickCount(tickSpacing);
    ...
    static getTickArrayStartIndexByTick(tickIndex, tickSpacing) {
        return this.getTickArrayBitIndex(tickIndex, tickSpacing) * tickQuery_1.TickQuery.tickCount(tickSpacing);
    }
    static getTickArrayOffsetInBitmapByTick(tick, tickSpacing) {
        const multiplier = tickSpacing * exports.TICK_ARRAY_SIZE;
    ...
    static getNextTickArrayStartIndex(lastTickArrayStartIndex, tickSpacing, zeroForOne) {
        return zeroForOne
            ? lastTickArrayStartIndex - tickSpacing * exports.TICK_ARRAY_SIZE
            : lastTickArrayStartIndex + tickSpacing * exports.TICK_ARRAY_SIZE;
    }
    ...
    static getInitializedTickArrayInRange(tickArrayBitmap, exTickArrayBitmap, tickSpacing, tickArrayStartIndex, expectedCount) {
        const tickArrayOffset = Math.floor(tickArrayStartIndex / (tickSpacing * exports.TICK_ARRAY_SIZE));
        return [
    ...
    static getAllInitializedTickArrayStartIndex(tickArrayBitmap, exTickArrayBitmap, tickSpacing) {
        ...
        return TickUtils.searchHightBitFromStart(tickArrayBitmap, exTickArrayBitmap, 0, exports.TICK_ARRAY_BITMAP_SIZE, tickSpacing);
    }
    static getAllInitializedTickArrayInfo(programId, poolId, tickArrayBitmap, exTickArrayBitmap, tickSpacing) {
        ...
        for (const startIndex of allInitializedTickArrayIndex) {
            const { publicKey: address } = (0, pda_1.getPdaTickArrayAddress)(programId, poolId, startIndex);
            result.push({
    ...
    static searchLowBitFromStart(tickArrayBitmap, exTickArrayBitmap, currentTickArrayBitStartIndex, expectedCount, tickSpacing) {
        ...
        const tickCount = tickQuery_1.TickQuery.tickCount(tickSpacing);
    ...
    static checkIsOutOfBoundary(tick) {
        return tick < constants_1.MIN_TICK || tick > constants_1.MAX_TICK;
    }
    static nextInitTick(tickArrayCurrent, currentTickIndex, tickSpacing, zeroForOne, t) {
        const currentTickArrayStartIndex = tickQuery_1.TickQuery.getArrayStartIndex(currentTickIndex, tickSpacing);
        ...
        if (zeroForOne) {
            ...
            if (!t)
                offsetInArray = offsetInArray + 1;
            while (offsetInArray < exports.TICK_ARRAY_SIZE) {
                if (tickArrayCurrent.ticks[offsetInArray].liquidityGross.gtn(0)) {
                    return tickArrayCurrent.ticks[offsetInArray];
                }
                offsetInArray = offsetInArray + 1;
    ...
    static firstInitializedTick(tickArrayCurrent, zeroForOne) {
        if (zeroForOne) {
            let i = exports.TICK_ARRAY_SIZE - 1;
            while (i >= 0) {
            ...
            let i = 0;
            while (i < exports.TICK_ARRAY_SIZE) {
                if (tickArrayCurrent.ticks[i].liquidityGross.gtn(0)) {
                    return tickArrayCurrent.ticks[i];
                }
                i = i + 1;
