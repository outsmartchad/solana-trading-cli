...
import { AddressLookupTableAccount, Keypair, PublicKey, Transaction, TransactionMessage, } from '@solana/web3.js';
import { TxVersion, } from '../base';
...
export async function getMultipleAccountsInfo(connection, publicKeys, config) {
    ...
    if (batchRequest) {
        ...
        const unsafeResponse = await (await Promise.all(_batch.map(async (i) => 
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        await connection._rpcBatchRequest(i)))).flat();
 ...
 */
export function forecastTransactionSize(instructions, signers) {
 ...
 */
export async function simulateMultipleInstruction(connection, instructions, keyword, batchRequest = true) {
...
export function parseSimulateLogToJson(log, keyword) {
...
export function parseSimulateValue(log, key) {
...
export async function simulateTransaction(connection, transactions, batchRequest) {
    let results = [];
    if (batchRequest) {
        ...
        results = await (await Promise.all(reqData.map(async (i) => {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            const d = await connection._rpcBatchRequest(i);
...
export async function splitTxAndSigners({ connection, makeTxVersion, innerTransaction, lookupTableCache, computeBudgetConfig, payer, }) {
    const lookupTableAddressAccount = lookupTableCache ?? {};
    const allLTA = [
        ...new Set(innerTransaction.map((i) => (i.lookupTableAddress ?? []).map((ii) => ii.toString())).flat()),
    ];
    const needCacheLTA = [];
...
export async function getMultipleLookupTableInfo({ connection, address, }) {
    const dataInfos = await getMultipleAccountsInfo(connection, [...new Set(address.map((i) => i.toString()))].map((i) => new PublicKey(i)));
    const outDict = {};
