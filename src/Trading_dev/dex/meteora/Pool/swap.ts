 ...
 */
export async function swap(side: string = "buy", tokenAddress: string, buyAmountInSOL: number = 0.1, sellPercentage: number = 100) {
  let swapYtoX = true, decimalY:number, decimalX:number, inToken:PublicKey, outToken:PublicKey, swapAmount:any;
  ...
  if(side === "buy"){
    ...
    if(dlmmPool.tokenY.publicKey.toBase58() === wsol){
      inToken = dlmmPool.tokenX.publicKey;
      outToken = dlmmPool.tokenY.publicKey;
      const balance = await getSPLTokenBalance(connection, inToken, wallet.publicKey);
