...
export async function getCurrentMarketCap(tokenAddress:string):Promise<number> {
    let priceInUSD:number = await getCurrentPriceInUSD(tokenAddress); 
    const tokenSupply:any = (await connection.getTokenSupply(new PublicKey(tokenAddress)))
