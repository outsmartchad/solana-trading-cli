import { 
  ...
  export async function sendBundle(latestBlockhash: string, transaction: VersionedTransaction, mint: PublicKey, masterKeypair: Keypair) {
  ...
  try {
    ...
    logger.info(
      {
        Pumpfun:`https://pump.fun/${mint.toBase58()}`
      },
    );
